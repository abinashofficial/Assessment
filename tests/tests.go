...
func Start() {
	...
	// Test Start Time
	startTimeEpoch := utils.CurrentMillis()

	// Get Current Branch Name
	branchName := utils.GetCurrentBranchName()

	// Run tests and store the exit code
	exitCode := runTests(packages)

	// Test End Time
	endTimeEpoch := utils.CurrentMillis()
...
func generateDetailedReport(file, branchName string, totalTime time.Duration) (CovReport, error) {
	...
	for _, profile := range profiles {
		...
		fileName := strings.Split(profile.FileName, "Assessment/")
	...
	covReport := CovReport{Module: "CRUD", RepoTotalLines: repoTotalLines,
	...
	html := `<!DOCTYPE html>
				...
				<body>
					<h4 class="text-center">CRUD Unit Testing Coverage Report</h4>
					<br>
	...
	out, err := os.Create("CRUDTestReport.html")
	if err != nil {
		panic(err)
	}
